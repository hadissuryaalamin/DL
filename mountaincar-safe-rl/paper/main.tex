...
on top of those bases, giving 18 constrained configurations over 3 seeds each.
...
\begin{enumerate}
    ...
    \item We adopt a disciplined \emph{two-stage} hyperparameter protocol (base screening, then
          a safety sweep on the top-3 bases) and show that the base's unconstrained cost --- not
...
The initialization $\ell_0$ and step size $\alpha_\lambda$ are tuned in Stage~2 (Section~\ref{sec:setup}).
...
P1--P6), each over 3 seeds. We rank bases by the stability-aware score
...
\emph{passive} speeding cost for each base --- computed with Eq.~\eqref{eq:cost} but never used in
...
(configs S1--S18), with $d{=}15$ fixed, again over 3 seeds. A configuration \emph{passes} if its
...
\texttt{python -m scripts.run\_stage2 --seeds 0 1 2}; the latter auto-selects the top-3 bases from
...
Table~\ref{tab:stage1} and Figure~\ref{fig:stage1} report the six unconstrained PPO bases.
...
\begin{table}[t]
  \centering
  \caption{Stage 1 (unconstrained PPO) base screening. Final-window (last 10 epochs) mean$\pm$std
  over 3 seeds. ``Avg cost'' is logged passively (not optimized). Score $=
  \text{mean}-\text{std}$ of return; the top-3 solving bases are carried into Stage~2.}
  \label{tab:stage1}
  \begin{tabular}{lccccccc}
    ...
    P6 & $10^{-3}$ & 0.01 & $47.8 \pm 0.2$    & $15.0$ & 100\% & $47.6$   & \checkmark \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{Stage 1 base selection: final return vs.\ passive cost (3-seed mean). Green circles are
...
Figure~\ref{fig:stage2} plots all 18 constrained configurations in the cost--return plane.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.6\linewidth]{fig5_stage2_scatter.pdf}
  \caption{Stage 2: final cost vs.\ return for all 18 Safe-PPO configurations (3-seed mean),
  colored by base. Filled circles pass (cost $\le 15$, no collapse); crosses fail. All P4-based
  configs (red) sit far past the budget; the feasible passing configs are built on the low-cost
  bases P6 and P5.}
  \label{fig:stage2}
\end{figure}

\subsection{The winning policy}
Figure~\ref{fig:winner} shows the winner's training dynamics. Return rises to $\approx 47$ within
...
\begin{figure}[t]
  ...
  \end{subfigure}
  \caption{Winning Safe~PPO (S6) dynamics, 3-seed mean$\pm$std. The constraint becomes binding
  early; $\lambda$ peaks then relaxes as cost converges to the budget with no return loss.}
  \label{fig:winner}
\end{figure}

\subsection{Comparison and summary}
Figure~\ref{fig:curves} compares the three agents' shaped return as a function of environment
...
constraint costs no measurable return on this task. Table~\ref{tab:final} summarizes final-window
performance.
...
\begin{figure}[t]
  ...
  (S6). Mean$\pm$std across 3 seeds. The two PPO variants are nearly indistinguishable.}
  \label{fig:curves}
\end{figure}
...
\begin{table}[t]
  ...
  (\texttt{--seeds 0 1 2}). Final return is averaged over the last 50 episodes (DQN) or last
  10 epochs (PPO, Safe~PPO). PPO is the selected base P6; Safe~PPO is the winner S6.}
  \label{tab:final}
  \begin{tabular}{lcccc}
    \toprule
...
operating band, which PID-Lagrangian \cite{stooke2020responsive} is designed to widen by damping
...
just above the achievable minimum. A hard-constraint variant (e.g.\ CPO \cite{achiam2017cpo})
would project the policy update onto the feasible set; we leave that comparison to future work.

\paragraph{Limitations.}
(i)~We screen with 3 seeds. The winner is stable ($47.3 \pm 0.3$), but several Stage~2 configs
show high across-seed variance (e.g.\ collapse on a single seed), so the winner should be re-run
with 5 seeds before being treated as final --- our selection script flags this explicitly.
...
pedagogically clear. Future work includes a 5-seed confirmation of the winner, a PID-Lagrangian
